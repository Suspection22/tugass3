...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Latihan MockUp API Get, Post, Edit, Delete',
      theme: ThemeData(
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future manuallyInputData(String title, String body) async {
    // Fungsi ini menambahkan data ke dalam list data secara manual
    final Map<String, dynamic> newData = {
      'id': data.length + 1, // Sediakan ID yang unik (misalnya, jumlah data + 1)
      'title': title,
      'body': body,
      'userId': 1,
    };
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Mock API (Daniel Alexander Papilaya)'),
      ),
          ...
          showDialog(
            ...
            builder: (BuildContext context) {
              return AddDialog(
                onAdd: (title, body) {
                  postData(title, body);
                  manuallyInputData(title, body); // Tambahkan data secara manual
                },
              );
            },
          );
